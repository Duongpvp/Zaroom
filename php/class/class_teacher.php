...
<body>
    ...
    <section>
        <div style="padding-top: 6%; " class="heading_main text_align_center">

            <?php
            echo '<h1 style="border:2px none;border-radius: 5px;padding:0.5%; color:white;"> Xin chào : ' . $name_user . '</h1>';
            ?>
        </div>
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "
                    <div class='class-box'>
                    <a class='btn btn-2 color-green' href='./class_in_class_teacher.php?nameCla=$row[name_class]&idClass=$row[id_class]' style='color:white; font-size:25px;margin:1%;height:150px;
    ...
    } else echo "<h1 class='heading_main text_align_center' style='padding-left: 450px;font-size:30px'>Bạn chưa tham gia lớp học nào!</h1>";
    echo "</div>";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($kt->num_rows > 0) {
                    ...
                    echo "<script type='text/javascript'>$('.heading_main text_align_center').load(</script>";
                }
            }
        } else {

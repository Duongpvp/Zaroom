...
<?php
                          if(isset($_POST['submit'])){    
                            ...
                            $query = mysqli_query($conn, "SELECT * FROM users WHERE email='$teacher'");
                            if (mysqli_num_rows($query) > 0) {
                            $row = mysqli_fetch_assoc($query);
                            $sql = "UPDATE class SET email_teacher='$teacher' WHERE id_class=$_GET[idClass]";
                  ...
                  <div class="col-lg-7 col-md-8 mb-4">
                    <div class="card h-100">
                      <div class="card-header d-flex justify-content-center">
                        <h4 class="card-title m-9 me-2 align-center">Sửa lớp học</h4>
                      </div>
                      <div class="card-body">
                        <div class="row">
                        <form id="formAccountSettings" method="POST">
                          ...
                          <div class="mt-2">
                            <button type="submit" class="btn btn-primary me-2" name="submit" value="submit">Lưu thay đổi</button>
                    ...
                    <div class="col-lg-5 col-md-4 mb-4">
                      <div class="card h-100">
                        <div class="card-header d-flex justify-content-center">
                            <h3 class="card-title m-9 me-2 align-center">Thông tin lớp học</h3>
                        </div>
                      <div class="card-body">
                          <div class="list-user">
                            <?php
                                    ...
                                    $query = mysqli_query($conn, "SELECT * FROM class
                                        WHERE class.id_class=$_SESSION[classID]");
                                    if (mysqli_num_rows($query) > 0) {
                                        echo '<div class="row">
                                                <div class="col-lg-2">ID</div>
                                                <div class="col-lg-5">Tên</div>
                                                <div class="col-lg-5">Teacher</div>
                                                <hr class="my-0" />
                                            </div>';
                                        while ($row = $query->fetch_assoc()) {
                                            echo '<div class="row">
                                                <div class="col-lg-2">'.$row["id_class"].'</div>
                                                <div class="col-lg-5">'.$row["name_class"].'</div>
                                                <div class="col-lg-5">'.$row["email_teacher"].'</div>
                                            </div>';
                                        }
                                    }                                                           
                            ?>
